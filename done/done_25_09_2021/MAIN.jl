                                ...
                                for t=Seasons[q][end-1]:-1:Seasons[q][1]-1
                                    if length(Part[t+1]) < nbRealization
                                        ...
                                        if q in Dry_index
                                            ...
                                            Part[t+1], fine_cut_counter = Type4_cut(MasterSubData,Part,SPC,SPSC,data,t,xxval,tthetaval,Refine,eps,SP,k);
                                        end
                                        PartsCountC[t+1]=length(PartC[t+1]);
                                        PartsCount[t+1]=length(Part[t+1]);
                                    end
    ...
    if PARAMS[1] == "PrePro" || PARAMS[1] == "Tree_Parts"
        ...
        while true
            ...
            if TotReal > phase3 && PARAMS[1] == "PrePro"
                ...
                nbSP, SP, objval, xval, thetaval, UB_SP, optimalx_1, ETGF, LB, Local_UB, UB, ignore, eps, LBs, TIMEOUT, indicator = initialize(nbRealization,nbStages,nbHydroP,nbSP_F);
                while true
